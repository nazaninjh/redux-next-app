...
import { selectUserById, useEditUserMutation } from './../../lib/features/users/usersSlice';
...
import { 
    useAddNewUserMutation, selectAllUsers,
    useGetAllUsersQuery
...
export default function EditUser({ id }) {
  ...
  const [emailState, emailDispatch] = useReducer(emailAction, {
    email: wantedUser? wantedUser.email: '',
    emailValid: false,
    emailFocus: false
  });
  const [pwdState, pwdDispatch] = useReducer(pwdAction, {
    pwd:  wantedUser? wantedUser.password: '',
    pwdValid: false,
    pwdFocus: false
  });
  ...
  const validateEdit = async () => {
    await data;
        users.map(user => {
            ...
            if (userState.userName === wantedUser.name &&
                       emailState.email === wantedUser.email) {
                setErrMsg(null);
                setIsOk(true);
            } else if (userState.userName === wantedUser.name) {
                setErrMsg(null);
                setIsOk(true);
            } else if (emailState.email === wantedUser.email) {
                setErrMsg(null);
                setIsOk(true);
            }
        })   
  ...
  const handleSubmit = async (e) => {
    ...
    // await users;
   await validateEdit();
   if (wantedUser.name === userState.userName &&
    wantedUser.email === emailState.email &&
    wantedUser.password === pwdState.pwd &&
    wantedUser.password === pwdmatchState.pwdmatch) {
